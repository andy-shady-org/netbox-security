...
class NatPoolMemberTable(NetBoxTable):
    ...
    tags = TagColumn(
        url_name='plugins:netbox_security:natpoolmember_list'
    )

    class Meta(NetBoxTable.Meta):
        model = NatPoolMember
        fields = ('pk', 'name', 'pool', 'address', 'prefix', 'address_range', 'source_ports', 'destination_ports', 'tags')
        default_columns = (
            'pk', 'name', 'address', 'prefix', 'address_range', 'source_ports', 'destination_ports'
        )
