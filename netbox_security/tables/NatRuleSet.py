...
class NatRuleSetTable(NetBoxTable):
    ...
    tags = TagColumn(
        url_name='plugins:netbox_security:natruleset_list'
    )

    class Meta(NetBoxTable.Meta):
        model = NatRuleSet
        fields = ('pk', 'name', 'description', 'nat_type', 'rule_count', 'source_zones', 'destination_zones', 'direction', 'tags')
        default_columns = (
            'pk', 'name', 'description', 'nat_type', 'rule_count', 'direction'
        )
...
class NatRuleSetDeviceAssignmentTable(NetBoxTable):
    ...
    ruleset = tables.Column(
        verbose_name=_('NAT Ruleset'),
        linkify=True
    )
