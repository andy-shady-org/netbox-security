...
class SecurityZoneSerializer(NetBoxModelSerializer):
    url = HyperlinkedIdentityField(view_name='plugins-api:netbox_security-api:securityzone-detail')
    tenant = TenantSerializer(nested=True, required=False, allow_null=True)

    class Meta:
        model = SecurityZone
        fields = ('id', 'url', 'display', 'name', 'description', 'tenant',
...
class SecurityZoneAssignmentSerializer(NetBoxModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'url', 'display', 'zone', 'assigned_object_type', 'assigned_object_id', 'assigned_object',
            'created', 'last_updated',
        ]
        brief_fields = ('id', 'url', 'display', 'zone', 'assigned_object_type', 'assigned_object_id')
